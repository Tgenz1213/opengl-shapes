...
// cylinder: min sector = 3, min stack = 1
Cylinder cylinder1(1, 1, 2, 36, 8, false);  // baseRadius, topRadius, height, sectors, stacks, non-smooth (flat) shading
Cylinder cylinder2(1, 1, 2, 36, 8);         // baseRadius, topRadius, height, sectors, stacks, smooth shading (default)
...
void displayCB()
{
    ...
    // bind vbo for flat cylinder (left)
    glBindBuffer(GL_ARRAY_BUFFER, vboId1);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, iboId1);

    // set attrib arrays using glVertexAttribPointer()
    int stride = cylinder1.getInterleavedStride();
    ...
    // left and center cylinders do not use texture
    glUniform1i(uniformTextureUsed, 0);

    // draw right cylinder
    glDrawElements(GL_TRIANGLES,                // primitive type
                   cylinder1.getIndexCount(),   // # of indices
                   GL_UNSIGNED_INT,             // data type
                   (void*)0);                   // ptr to indices

    // bind vbo for smooth cylinder (center and right)
    glBindBuffer(GL_ARRAY_BUFFER, vboId2);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, iboId2);

    // set attrib arrays using glVertexAttribPointer()
    stride = cylinder2.getInterleavedStride();
    glVertexAttribPointer(attribVertexPosition, 3, GL_FLOAT, false, stride, 0);
    glVertexAttribPointer(attribVertexNormal, 3, GL_FLOAT, false, stride, (void*)(3 * sizeof(float)));
    glVertexAttribPointer(attribVertexTexCoord, 2, GL_FLOAT, false, stride, (void*)(6 * sizeof(float)));

    // set matrix uniforms for center cylinder
    ...
    // set matric uniforms for right cylinder
    ...
    // right cylinder is rendered with texture
    glUniform1i(uniformTextureUsed, 1);
